...
{
    internal static async Task SendPing(this PingStatsData data, PingOptions? options = default, CancellationToken token = default)
    {
        options ??= new();
        using Ping P = new();
        PingReply reply = await P.SendPingAsync(data.Address, options.Timeout);
        data.UpdateRecords(reply.RoundtripTime, options);
    }

    private static void UpdateRecords(this PingStatsData data, long roundtripTime, PingOptions options)
    {
        data.Pings.Add(roundtripTime);
        while (data.Pings.Count > options.MaximumPingsCount)
            data.Pings.Remove(0);
    }
